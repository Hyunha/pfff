...
(* Racine carr馥 enti鑽e par la m騁hode de Newton (enti鑽e par d馭aut). *)

(* Th駮r鑪e: la suite xn+1 = (xn + a/xn) / 2 converge vers la racine *)
(* carr馥 enti鑽e de a par d馭aut, si on part d'une valeur x0 *)
(* strictement plus grande que la racine de a, sauf quand a est un *)
(* carr� - 1, cas auquel la suite alterne entre la racine par d馭aut *)
(* et par exc鑚. Dans tous les cas, le dernier terme de la partie *)
(* strictement d馗roissante de la suite est le r駸ultat cherch�. *)
...
let sqrt_nat rad off len =
 ...
 (* Racine carr馥 suppos馥 cand = "|FFFF .... |" *)
 let cand = make_nat cand_len in
 (* Am駘ioration de la racine de d駱art:
    ...
    (la moiti� du nombre de bits significatifs dans les deux premiers
     digits du radicande 騁endu � une longueur paire).
